...
def draw_pro_chart(candles, pair, entry_time, direction, payout, result=None, confidence=0,
    ...
    def paste_region(base_img, overlay_rgba, box):
        """
        Composite an RGBA overlay onto base_img (RGB) but only within `box`
        instead of running alpha_composite across the whole canvas.
        A full-canvas alpha_composite on a ~4700x2500 supersampled image is
        expensive (profiled at ~0.4s per call) and adds up fast on low-CPU
        hosts - this keeps the same visual result for a fraction of the cost.
        """
        ...
        for pair in pairs:
            ...
            if bias:
                ...
                if fmt == 2:
                    ...
                    chart_path = safe_draw_pro_chart(candles, pair, entry_t, direction, payout, result=None, confidence=score)
                ...
                if fmt == 2:
                    ...
                    if win1:
                        ...
                        chart_path = safe_draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="WIN", confidence=score)
                        ...
                        if win2:
                            ...
                            chart_path = safe_draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="MTG WIN", confidence=score)
                            result_msg = build_result_format3(
                            ...
                            st.signal_history.append({'pair': pair, 'direction': direction, 'time': entry_t, 'result': 'LOSS', 'type': 'NON-MTG'})
                            chart_candles = candles3[-30:] if len(candles3) > 30 else candles3
                            chart_path = safe_draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="LOSS", confidence=score)
                            result_msg = build_result_format3(
