...
class UserTelegramSender:
    ...
    def send_file(self, chat_id, file_path, caption, add_bold=False):
        if not self.ready:
           print(f"{Fore.RED}[!] User Telegram not ready.{Style.RESET_ALL}")
           return False
        async def _send():
           entities = self._build_entities(caption, add_bold=add_bold)
...
class TelegramSender:
    ...
    def start_with_bot_token(self, api_id, api_hash, bot_token):
        ...
        async def init():
            try:
                ...
                # Capture it so the main thread can see the REAL reason
                # (e.g. FloodWaitError) instead of a generic timeout.
                self._start_error = _init_err
                raise
        ...
        def run_loop():
            ...
            try:
                ...
                pass  # already recorded in self._start_error
        t = threading.Thread(target=run_loop, daemon=True)
        t.start()
        timeout = 30
        start_time = time.time()
        while not self.ready and self._start_error is None and time.time() - start_time < timeout:
            time.sleep(0.5)
        if self._start_error is not None:
            # Re-raise the real error (e.g. FloodWaitError with .seconds)
    ...
    def delete_message(self, chat_id, msg_id):
        async def _delete():
            return await self.client.delete_messages(chat_id, msg_id)
...
async def run_advanced_checker(update: Update, context: ContextTypes.DEFAULT_TYPE, signals, mtg_level, date_str, user_utc, payout_filter):
    ...
    def fetch_pair(api):
        for attempt in range(3):
            try:
                url = f"https://mrbeaxt.site/Qxapi/qx.php?pair={api}&timeframe=M1&count=1500"
                ...
                print(f"Attempt {attempt+1} failed for {api}: {e}")
                ttime.sleep(2)
...
def fetch_candles_mrbeaxt(pair: str, count: int = 300, timeframe: str = "1m"):
    """
    Fetch candles using the quotex-proxy API.
    timeframe: '1m' or '5m' (default '1m')
    ...
    if "_OTC" in pair or "-OTC" in pair:
        ...
        if not api_pair.endswith("-OTC"):
            api_pair = api_pair.replace("OTC", "-OTC")
            if not api_pair.endswith("-OTC"):
                api_pair += "-OTC"
    else:
        api_pair = pair.upper()

    print(f"🔍 FETCH: {original} → API: {api_pair} (tf={timeframe})")

    url = f"https://ikszeynptbmwkaaldfad.supabase.co/functions/v1/quotex-proxy?symbol={api_pair}&interval={timeframe}&limit={count}:qx_fxbd1pmgumxe8xo8j9mgz8nbeiabq3p3"
    headers = {
        "apikey": SUPABASE_ANON_KEY,
        "Authorization": f"Bearer {SUPABASE_ANON_KEY}"
    }
...
def handle_multi_result(uid: int, pair: str, entry_dt: datetime, direction: str, payout: int,
    ...
    candles, _, _ = fetch_candles_mrbeaxt(pair, count=50)
    if not candles: return

    entry_epoch = int((entry_dt - timedelta(hours=5)).timestamp())
    c1 = next((c for c in candles if c['time'] == entry_epoch), None)
    if not c1: return
    ...
    if win1:
        ...
        candles2, _, _ = fetch_candles_mrbeaxt(pair, count=50)
        if not candles2: return

        mtg_epoch = int((entry_dt + timedelta(minutes=1) - timedelta(hours=5)).timestamp())
        c2 = next((c for c in candles2 if c['time'] == mtg_epoch), None)
        if not c2: return
...
def _send_via_ptb_fallback(uid: int, context: ContextTypes.DEFAULT_TYPE, text: str):
    """
    Fallback path using python-telegram-bot's own client (MAIN_LOOP) instead of
    Telethon. Used whenever the Telethon `sender` fails/times out, so the user
    still gets *something* in chat instead of the loop going silent.
    """
    try:
        if MAIN_LOOP is None:
            return None
        fut = asyncio.run_coroutine_threadsafe(
            context.bot.send_message(chat_id=uid, text=text), MAIN_LOOP
        )
        return fut.result(timeout=15)
    except Exception as _ptb_err:
        print(f"[MULTI STATUS PTB FALLBACK ERROR] {_ptb_err}")
        return None
...
def update_multi_control_panel(uid: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg_id:
        ...
        asyncio.run_coroutine_threadsafe(edit(), MAIN_LOOP)
...
def multi_nonstop_loop(uid: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    while context.user_data.get('multi_running', False):
        ...
        try:
            ...
            candles1, _, _ = fetch_candles_mrbeaxt(pair, count=50)
            wins = context.user_data['multi_stats']['wins']
            losses = context.user_data['multi_stats']['losses']

            if candles1:
                entry_epoch = int((entry_dt - timedelta(hours=5)).timestamp())
                c1 = next((c for c in candles1 if c['time'] == entry_epoch), None)
                if c1:
                    win1 = (c1['close'] > c1['open']) if direction == "CALL" else (c1['close'] < c1['open'])
                    c1_color = "green" if c1['close'] > c1['open'] else "red"
                    ...
                    if win1:
                        ...
                        chart_path = safe_draw_pro_chart(candles1, pair, entry_time, direction, payout,
            ...
            if candles2:
                ...
                if c2:
                    ...
                    if win2:
                        ...
                        chart_path = safe_draw_pro_chart(candles2, pair, entry_time, direction, payout,
...
def start_auto_signal_session(uid, strategy_id, context):
    ...
    st.auto_timeframe = tf   # we need to add this attribute to UserState (optional, we'll just pass)
...
def send_result_message(st, user_sender, fmt, pair, entry_t, direction, payout, result_type,
    ...
    if fmt == 3:
        ...
        if chart_path and os.path.exists(chart_path):
            user_sender.send_file(st.target_chat, chart_path, result_text, add_bold=True)
            try: os.remove(chart_path)
            except: pass
        else:
            user_sender.send_bold_message(st.target_chat, result_text)
    elif fmt == 2:
        if result_type == "WIN":
        ...
        try:
            ...
            for pair in pairs:
                ...
                try:
                    ...
                    # Log instead of swallowing silently — a real bug here used
                    # to look identical to "no setup found", making failures
                    # invisible.
                ...
                if bias:
                    ...
                    if fmt == 2:
                        ...
                        if chart_path and os.path.exists(chart_path):
                            user_sender.send_file(st.target_chat, chart_path, signal_text, add_bold=True)
                            try: os.remove(chart_path)
                            except: pass
                        else:
                            user_sender.send_bold_message(st.target_chat, signal_text)
                    ...
                    if st.stop_requested:
                        break
                    candles2, _, _ = fetch_candles_mrbeaxt(pair, count=50, timeframe=tf)
                    if not candles2:
                        continue
                    entry_epoch = int((entry_dt - timedelta(hours=5)).timestamp())
                    first = None
                    for c in candles2:
                        if abs(c['time'] - entry_epoch) < (tf_minutes * 30):
                            first = c
                            break
                    if not first:
                        continue
                    ...
                    if win1:
                        ...
                        # Send result using format
                        send_result_message(st, user_sender, fmt, pair, entry_t, direction, payout, "WIN",
                        ...
                        if st.stop_requested:
                            break
                        candles3, _, _ = fetch_candles_mrbeaxt(pair, count=50, timeframe=tf)
                        if not candles3:
                            continue
                        mtg_epoch = int((entry_dt + timedelta(minutes=tf_minutes) - timedelta(hours=5)).timestamp())
                        second = None
                        for c in candles3:
                            if abs(c['time'] - mtg_epoch) < (tf_minutes * 30):
                                second = c
                                break
                        if not second:
                            continue
                        ...
                        if win2:
                            ...
                            send_result_message(st, user_sender, fmt, pair, entry_t, direction, payout, "MTG WIN",
                            ...
                            send_result_message(st, user_sender, fmt, pair, entry_t, direction, payout, "LOSS",
                                                second['open'], second['close'], c2_color,
                                                st.session_stats['wins'], st.session_stats['losses'],
                                                tf_display, owner, candles3, confidence)
                            pair_cooldown[pair] = t.time() + 240
            ...
            _tb.print_exc()
            # Previously this error only went to the console — the chat went
            # completely silent whenever a send/analyze call failed, which is
            # why signals looked random ("kabhi aata hai kabhi nahi"). Now we
            # also notify the chat via the PTB bot so it's visible.
            notify_chat = getattr(st, 'target_chat', None) or uid
            _send_via_ptb_fallback(uid, context, f"⚠️ Auto signal error: {str(_loop_err)[:80]} – retrying...")
            t.sleep(3)
            continue
...
def main():
    global MAIN_LOOP   # <-- we need to write to the global
...
# ONLY this handler for testing
    ...
    # Future Signals – Strategy 4 (Pattern Replay)
...
if __name__ == "__main__":
   # keep_alive()   # start uptime server thread
    ...
    while True:
        try:
            main()
            # main() only returns if run_polling() exits cleanly (e.g. manual stop)
            print(f"{Fore.YELLOW}[!] Bot polling ended cleanly — exiting.{Style.RESET_ALL}")
            break
        except FloodWaitError as _flood_err:
            # Telegram itself is rate-limiting bot-token logins right now.
            # Retrying quickly makes this WORSE (can extend the block), so we
            # sleep for exactly the time Telegram asked for, plus a buffer.
            wait_s = getattr(_flood_err, 'seconds', 300) + 15
            print(f"{Fore.RED}[FLOOD WAIT] Telegram is rate-limiting login. "
                  f"Waiting {wait_s}s before retrying — do NOT redeploy manually during this time.{Style.RESET_ALL}")
            time.sleep(wait_s)
            _restart_delay = 5  # reset normal backoff after a flood wait
            continue
        except Exception as _fatal_err:
            # A transient network error (e.g. Telegram getMe() timeout on startup)
            # used to crash the whole process (exit status 1) and rely on Render
            # to restart it from scratch, which is slow and can loop-crash if the
            # network blip is on Render's side too. Now we catch it, log it, and
            # restart main() in-process with a short backoff instead.
            print(f"{Fore.RED}[FATAL] Bot crashed: {_fatal_err}{Style.RESET_ALL}")
            _tb_main.print_exc()
            print(f"{Fore.YELLOW}[!] Restarting in {_restart_delay}s...{Style.RESET_ALL}")
            time.sleep(_restart_delay)
            # Simple capped backoff so a persistent outage doesn't spam-restart
