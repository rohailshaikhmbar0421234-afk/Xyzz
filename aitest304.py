...
def _update_status(uid: int, context: ContextTypes.DEFAULT_TYPE, status_text: str):
    """
    Update the status message with premium emoji icons using sender.edit_message.
    """
    ...
    msg_id = context.user_data.get('multi_status_msg_id')
    if msg_id:
        try:
            # Use sender.edit_message (sync, handles async internally)
            sender.edit_message(uid, msg_id, full_text)
            return
        except Exception:
            pass
...
def multi_nonstop_loop(uid: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    while context.user_data.get('multi_running', False):
        ...
        try:
            ...
            # ─── Catch any error, log, and continue ──────
...
def auto_signal_loop(uid, strategy_id, context):
    ...
    # ─── Get timeframe ──────────────────────────────────
    tf = context.user_data.get('auto_signal_timeframe', '1m')
    tf_display = 'M1' if tf == '1m' else 'M5'   # for display
    tf_minutes = 1 if tf == '1m' else 5

    owner = context.user_data.get('multi_owner_name', '@Rohailtrader')   # if you want custom owner in auto signal; you can also keep default.
    ...
    while st.running:
        ...
        for pair in pairs:
            ...
            if bias:
                signal_found = True
                # ─── Compute entry time based on timeframe ──
                now_pk = datetime.now(timezone.utc) + timedelta(hours=5)
                if tf == '5m':
                ...
                if fmt == 2:
                    ...
                    chart_path = safe_draw_pro_chart(candles, pair, entry_t, direction, payout,
                                                     result=None, confidence=score, tf=tf_display)
                    signal_text = build_signal_format3(pair, entry_t, direction, payout, owner_name=owner, tf=tf_display)
                    if chart_path and os.path.exists(chart_path):
                ...
                # ─── Result check ──────────────────────────────
                # Wait for timeframe candle close
                ...
                if not candles2:
                    continue
                entry_epoch = int((entry_dt - timedelta(hours=5)).timestamp())
                # Find the candle that matches the entry time (with tolerance)
                first = None
                for c in candles2:
                ...
                if win1:
                    ...
                    # Martingale: wait another timeframe
