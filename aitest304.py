...
def draw_pro_chart(candles, pair, entry_time, direction, payout, result=None, confidence=0,
    ...
    if is_result and result:
        ...
        for gi in glow_indices:
            gx = x_pos(gi)
            glow_draw.rectangle([gx - glow_half_w, PRICE_TOP, gx + glow_half_w, PRICE_BOTTOM], fill=glow_color)
        glow_overlay = glow_overlay.filter(ImageFilter.GaussianBlur(radius=F(16)))
    ...
    if show_trendline and n > 2:
        ...
        if show_channel:
            ...
            # extend the channel beyond the candle range so it crosses the canvas edges,
            # like the reference chart's diagonal lines
            extend = (n - 1) * 0.35
    ...
    if is_result:
        ...
        draw.text((box_x1 + F(8), box_y1 + F(30)), f"{pair} | M1", fill=TXT_GRAY, font=font_small)
    ...
    if is_result and result:
        ...
        overlay = Image.new('RGBA', (W, H), (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        od.rectangle([box_x1, box_y1, box_x2, box_y2], fill=box_color, outline=outline_color, width=3 * SS)
        img = Image.alpha_composite(img.convert('RGBA'), overlay).convert('RGB')
        draw = ImageDraw.Draw(img)
        ...
        ld.rectangle([label_x1, label_y1, label_x2, label_y2], fill=(10, 10, 26, 220), outline=outline_color, width=2 * SS)
        img = Image.alpha_composite(img.convert('RGBA'), label_overlay).convert('RGB')
        draw = ImageDraw.Draw(img)
    ...
    wm_draw.text((wm_x, wm_y), watermark_text, fill=wm_color, font=font_watermark)
...
def build_signal_format3(pair, entry_time, direction, payout):
    """
    ...
    msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🐊 𝙰𝚄𝚃𝙾 𝚁𝙴𝚂𝚄𝙻𝚃 𝙲𝙷𝙴𝙲𝙺\n"
        f"⏳ 𝚁𝙴𝚂𝚄𝙻𝚃 𝙰𝙵𝚃𝙴𝚁 1 𝙼𝙸𝙽𝚄𝚃𝙴\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🐅 𝙾𝚆𝙽𝙴𝚁 ➜ @Rohailtrader"
    )
    return msg

def build_result_format3(pair, entry_time, direction, result_type, open_price, close_price, candle_color, wins, losses):
    """
    Format 3 result message – complete bold, dynamic direction emoji, centered HIT GALE.
    """
    if direction == "CALL":
        dir_display = "🍋 𝙲𝙰𝙻𝙻📉"
    else:
...
def _chk2_parse_signals(signals_text: str, from_offset: int, base_date: str) -> list:
    ...
    for line in lines:
        ...
        rest = re.sub(dir_pattern, '', rest, flags=re.IGNORECASE)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['strategy_active'] = False
    context.user_data['state'] = None
    # Clear trading mode so next start defaults to OTC
...
def auto_signal_loop(uid, strategy_id, context):
    ...
    # 🔥 🔥 🔥 YAHAN CHANGE HAI 🔥 🔥 🔥
    # Pehle: pairs = bot.pairs
    # Ab: Market selection ke hisaab se pairs set karte hain
    ...
    while st.running:
        ...
        for pair in pairs:
            ...
            if bias:
                ...
                # ========================= SIGNAL SEND =========================
                if fmt == 2:
                    # Format 2: Text only (already bold)
                    signal_text = build_signal_format2(pair, entry_t, direction)
                    user_sender.send_bold_message(st.target_chat, signal_text)

                elif fmt == 3:
                    # Format 3: Pro chart + bold caption
                    chart_path = draw_pro_chart(candles, pair, entry_t, direction, payout, result=None, confidence=score)
                ...
                if fmt == 2:
                    # Format 2: Text only result (already bold)
                    if win1:
                    ...
                    # Format 3: Pro chart + bold caption result
                    if win1:
                        st.session_stats['wins'] += 1
                        st.signal_history.append({'pair': pair, 'direction': direction, 'time': entry_t, 'result': 'WIN', 'type': 'NON-MTG'})
                        chart_candles = candles2[-30:] if len(candles2) > 30 else candles2
                        chart_path = draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="WIN", confidence=score)
                        result_msg = build_result_format3(
                            pair, entry_t, direction, "WIN",
                            first['open'], first['close'],
                            "green" if first['close'] > first['open'] else "red",
                            st.session_stats['wins'], st.session_stats['losses']
                        )
                        if chart_path and os.path.exists(chart_path):
                            user_sender.send_file(st.target_chat, chart_path, result_msg, add_bold=True)
                            try:
                                os.remove(chart_path)
                            except:
                                pass
                        ...
                        if win2:
                            st.session_stats['wins'] += 1
                            st.signal_history.append({'pair': pair, 'direction': direction, 'time': entry_t, 'result': 'WIN', 'type': 'MTG'})
                            chart_candles = candles3[-30:] if len(candles3) > 30 else candles3
                            chart_path = draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="MTG WIN", confidence=score)
                            result_msg = build_result_format3(
                                pair, entry_t, direction, "MTG WIN",
                                second['open'], second['close'],
                                "green" if second['close'] > second['open'] else "red",
                                st.session_stats['wins'], st.session_stats['losses']
                            )
                            if chart_path and os.path.exists(chart_path):
                                user_sender.send_file(st.target_chat, chart_path, result_msg, add_bold=True)
                            ...
                            st.signal_history.append({'pair': pair, 'direction': direction, 'time': entry_t, 'result': 'LOSS', 'type': 'NON-MTG'})
                            chart_candles = candles3[-30:] if len(candles3) > 30 else candles3
                            chart_path = draw_pro_chart(chart_candles, pair, entry_t, direction, payout, result="LOSS", confidence=score)
                            result_msg = build_result_format3(
                                pair, entry_t, direction, "LOSS",
                                second['open'], second['close'],
                                "red" if second['close'] < second['open'] else "green",
                                st.session_stats['wins'], st.session_stats['losses']
                            )
                            if chart_path and os.path.exists(chart_path):
                                user_sender.send_file(st.target_chat, chart_path, result_msg, add_bold=True)
                    ...
                    # Format 1: Neon chart + bold result
                    if win1:
                        st.session_stats['wins'] += 1
