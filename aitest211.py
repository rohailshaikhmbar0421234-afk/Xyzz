...
class UserTelegramSender:
    ...
    def start(self):
        ...
        session_string = "1BJWap1sBuzzaO70mkejg7aBal3ZsSJ_1TJtlsXBQTgbMBDw6D-rjCj1PLeeausKEE9ry3gTz_0PizZlEcB3Nj5IB53B0VcugQJIDolO4ig7axfpBYaVoH1Ogs0wcpbiKn7m2W2zL5ZvBrlCRZbZYI7lbtYOTwb4u6f9rnydgPLyjr-2xhSBF-KQWEaV6zN_iokvtlt76O-gm5Gvql2VAEeIt70I5oqWLy1PbwdRdWhUh9awtCGx5npMdmO8PYUU1Rlw4Xc1b3DmS6_dQ_KCWzjg15pv4wrIqTUdm83L5vviaKWT14Xe8q4E8R9HalEHj-UFaEEfFEHyMB2TboInL-vySmwIJqfM="
        if not session_string:
            raise RuntimeError("TG_SESSION_STRING not set!")
...
def run_whiteout_signals(uid, context):
    """Whiteout Signal with display fix, real-time animation, and robust retry."""
    try:
        ...
        # Date range: last `days` days excluding today
        ...
        for pair in pairs:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        print(f"⚠️ {pair} HTTP {resp.status_code}, retry {attempt+1}/{max_retries}")
                        time.sleep(retry_delay)
                        retry_delay *= 2
                        continue
                    else:
                        print(f"⚠️ Skipping {pair} (HTTP {resp.status_code})")
                        break
                except Exception as e:
                    print(f"⚠️ Error fetching {pair}: {e}, retry {attempt+1}")
                    time.sleep(retry_delay)
                    retry_delay *= 2
                    continue
            ...
            for c in candles:
                ...
                try:
                    ...
                    high_p = float(c.get('h', 0))
                    low_p = float(c.get('l', 0))
            ...
            ema_lookup = {}
            if USE_TREND_FILTER and len(full_candles) >= 20:
                closes = [c['close'] for c in full_candles]
                ema_series = calc_ema(closes, 20)
                for idx, c in enumerate(full_candles):
                    if ema_series[idx] is not None:
                        dt = c['time']
                        key = (dt.strftime("%Y-%m-%d"), dt.strftime("%H:%M"))
                        ema_lookup[key] = ema_series[idx]
            ...
            for date in date_list:
                ...
                while current_min <= end_min:
                    ...
                    if candle and candle['body_pct'] > MIN_BODY_PCT:
                        if candle['is_doji']:
                            streak_color = None
                            streak_length = 0
                    ...
                    if streak_length >= REQUIRED_STREAK:
                        ok = True
                        if USE_TREND_FILTER:
                            ema_val = ema_lookup.get((date, hhmm))
                            if ema_val is not None:
                                prev_hhmm = f"{h:02d}:{m-1:02d}" if m > 0 else f"{h-1:02d}:59"
                                prev_ema = ema_lookup.get((date, prev_hhmm))
                                if prev_ema is not None:
                                    slope_up = ema_val > prev_ema
                                    if streak_color == 'green':
                                        if not (candle['close'] > ema_val and slope_up):
                                            ok = False
                                    elif streak_color == 'red':
                                        if not (candle['close'] < ema_val and not slope_up):
                                            ok = False
                                else:
                                    ok = False
                            else:
                        ...
                        if ok:
                            all_signals_raw.append((pair, hhmm))
                            streak_color = None
                            streak_length = 0
            ...
            processed += 1
            pct = int((processed / total_combinations) * 100) if total_combinations else 0
            ...
            try:
                sender.edit_message(uid, progress_id, 
                    f"⏳ Scanning {spin_char} {mins:02d}:{secs:02d}\n"
                    f"🔄 {pct}%  ({processed}/{total_combinations})\n"
                    f"📊 {pair}"
                )
            except:
                pass
        ...
        if not all_signals_raw:
            sender.edit_message(uid, progress_id, "❌ No Whiteout signals found.")
            return

        # Deduplicate
        seen = set()
        deduped = []
        for pair, time_str in all_signals_raw:
            key = (pair, time_str)
            if key not in seen:
                seen.add(key)
                deduped.append((pair, time_str))
        ...
        # Body: uppercase base + _otc (e.g., TRUUSD_otc)
        body_lines = []
        for pair, time_str in deduped:
            display_pair = pair.replace("-OTC", "") + "_otc"
            body_lines.append(f"☫ {display_pair}•{time_str} ;")
        body = "\n".join(body_lines)
        ...
        sender.edit_message(uid, progress_id, f"✅ Whiteout signals generated: {len(deduped)} found")
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        # Row 2: OTC LIVE MODE + Signal Checker
        ...
        # Row 5: CHECKER 2.0 + BACKTEST 2.0
        ...
        # Row 6: BLACKOUT FS + BLACKOUT CHECKER
        ...
        # Row 7: UTC Converter + Pair Payout%
        ...
        # Row 8: Market Trend + Candle Colors
        ...
        # Row 9: Text Formatter + Font Changer
        ...
        # Row 10: Trend Filter + Auto Signal
        ...
        # Row 11: AI Filter FS + AI Chart Analyzer
        ...
        # Row 13: SWAP C/P + About (About last)
    ...
    async def run():
        ...
        while trader.running:
            try:
                ...
                if not chosen:
                    # Active countdown to next candle scan — prevents frozen "NO SIGNAL" screen
                    nxt_scan = boundary + 60 - 3   # 3 s before the next boundary
                    while trader.running and _t.time() < nxt_scan:
                        left = int(round(nxt_scan - _t.time()))
                        await status(
                            f"🔍 {fancy_font('NO SIGNAL')}\n"
                            f"🤖 {trader.strategy}. {trader.strategy_name}\n"
                            f"⏰ {entry_label} — next scan in {left}s..."
                        )
                        await _aio.sleep(2)
                    reset_status()
                    continue
    ...
    try:
        ...
        for pair, candles in all_candles.items():
            ...
            for minute in range(start_min, end_min + 1):
                ...
                matches = []
                for hist_pat, next_dir in hist_patterns:
                    dist = euclidean_dist(current_pat, hist_pat)
                    matches.append((dist, next_dir))
...
def start_auto_signal_session(uid, strategy_id, context):
    ...
    # === GUARD: Prevent accidental call from Auto Trade ===
